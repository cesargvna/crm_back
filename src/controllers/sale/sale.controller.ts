...
export const getSalesBySubsidiary = asyncHandler(async (req: Request, res: Response) => {
  ...
  const whereClause: Prisma.SaleWhereInput = {
    ...
    ...(clientCategoryId ? { client: { clientCategoryId: clientCategoryId as string } } : {}),
    ...(saleDateFrom || saleDateTo
      ? {
          saleDate: {
            ...(saleDateFrom ? { gte: new Date(saleDateFrom as string) } : {}),
...
export const getSaleById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const priceTypeMap = await prisma.priceType.findMany({
    where: {
      id: {
        in: sale.saleDetails.map((d: any) => d.priceTypeId).filter(Boolean) as string[],
      },
    },
    include: {
      currency: true,
...
export const createSaleWithStockValidation = asyncHandler(async (req: Request, res: Response) => {
  const {
    saleDate,
    paymentType,
    paymentStatus,
  ...
  const generatedCode = await generateNextSaleCode(saleDate, tenantId, subsidiaryId);
...
export const createSaleAllowNegativeStock = asyncHandler(async (req: Request, res: Response) => {
  const {
    saleDate,
    paymentType,
    paymentStatus,
